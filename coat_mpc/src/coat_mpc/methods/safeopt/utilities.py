...
def linearly_spaced_combinations(bounds, num_samples) -> np.ndarray:
    """
...
def sample_gp_function(
    kernel, bounds, noise_var, num_samples, interpolation="kernel", mean_function=None
):
    """
    ...
    output = np.random.multivariate_normal(np.zeros(inputs.shape[0]), cov)
...
def plot_2d_gp(
    ...
    **kwargs
    ...
    axis.set_xlim([np.min(inputs[:, unfixed[0]]), np.max(inputs[:, unfixed[0]])])
...
def plot_3d_gp(
    ...
    **kwargs
    ...
    data = axis.plot(gp.X[:-1, unfixed[0]], gp.X[:-1, unfixed[1]], gp.Y[:-1, 0], "o")
    axis.plot(gp.X[-1, unfixed[0]], gp.X[-1, unfixed[1]], gp.Y[-1, 0], "ro")

    axis.set_xlim([np.min(inputs[:, unfixed[0]]), np.max(inputs[:, unfixed[0]])])

    axis.set_ylim([np.min(inputs[:, unfixed[1]]), np.max(inputs[:, unfixed[1]])])

    return surf, data


def plot_contour_gp(
    gp, inputs, predictions=None, figure=None, axis=None, colorbar=True, **kwargs
):
    """
    Plot a 3D gp with uncertainty.
    ...
    axis.set_xlim([np.min(inputs[slices[0]]), np.max(inputs[slices[0]])])

    axis.set_ylim([np.min(inputs[slices[1]]), np.max(inputs[slices[1]])])
